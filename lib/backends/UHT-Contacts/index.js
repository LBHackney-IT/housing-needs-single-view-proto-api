...
const { fetchCustomerSQL, searchCustomersBaseSQL, fetchActionDiariesNotesSQL } = loadSQL(
...
async function fetchActionDiariesNotes(id) {
  const [house_ref, person_no] = id.split('/');

  return (await db.request(fetchActionDiariesNotesSQL, [
    { id: 'house_ref', type: 'NVarChar', value: house_ref },
  ]));
}
...
let processNotes = function(results) {
  return results.map(record => {
    return {
      title: 'Note',
      text: record.action_comment,
      date: formatRecordDate(record.action_date),
...
let Backend = {
  ...
  fetchCustomerNotes: async function(id) {
    ...
    try {
      const results = await fetchActionDiariesNotes(id);
      const notes = processNotes(results);
      console.log(notes)
      return notes;
    } catch (err) {
      console.log(`Error fetching customers in UHT-Contacts: ${err}`);
