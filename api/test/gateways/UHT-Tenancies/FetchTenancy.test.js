const UHTTenanciesFetchRecord = require('../../../lib/gateways/UHT-Tenancies/FetchTenancy');

describe('UHTTenanciesFetchRecord gateway', () => {
  let db;
  let logger;
  ...
  const createGateway = (response, throwsError) => {
    ...
    return UHTTenanciesFetchRecord({
      db,
      logger
    });
  };
  ...
  it('catches and calls logger', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      'Error fetching tenancy in UHT-Tenancies: Error: Database error',
      dbError
    );
  });
});
